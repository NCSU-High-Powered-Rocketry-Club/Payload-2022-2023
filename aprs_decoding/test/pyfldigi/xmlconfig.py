...
config_dir = "C:\\Users\\HenkA\\fldigi.files\\"
...
class XmlConfig(object):
    ...
    def read(self):
        ...
        >>> # do some other stuff
        >>> xc.read()
        >>> xc['KISS_IO_PORT']  # Read some random setting named 'KISS_IO_PORT'
        '7342'
        """
        ...
        if not self.root.tag == "FLDIGI_DEFS":
            raise Exception(
                "Expected root tag to be 'FLDIGI_DEFS' but got {}".format(self.root.tag)
            )
    ...
    def __str__(self):
        ...
        >>> import pyfldigi
        >>> xc = pyfldigi.XmlConfig(config_dir)
        >>> str(xc)     # doctest: +ELLIPSIS
        '...'

        Real output is 'confirmexit : 1\nshow_tx_timer : 1\ntx_timeout : 5\nsaveconfig : 1\nnoisetest : 0\n...'
        """
        s = ""
    ...
    def __getitem__(self, key):
        ...
        >>> import pyfldigi
        >>> xc = pyfldigi.XmlConfig(config_dir)
        >>> xc['XMLRIGDEVICE']      # doctest: +ELLIPSIS
        'COM...'
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, str):
            ...
            raise TypeError(
                "Types supported are bool, str, float, or int.  If you are purposely setting another type, please cast it to one of these."
            )
        try:
