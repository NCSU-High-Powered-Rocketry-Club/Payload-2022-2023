...
class Client(object):
    ...
    >>> fldigi = client.Client(hostname='127.0.0.1', port=7362)  # <-- Constructor call
    >>> fldigi.name
    'fldigi'
    ...
    def __init__(self, hostname="127.0.0.1", port=7362, reset=True, log=False):
        ...
        self.port = port
        self.logger.debug(
            "Attempting to connect to connect to fldigi at IP address={}, port={}, via XMP-RPC".format(
...
if __name__ == "__main__":
    ...
    fldigi_client = client.Client(hostname="192.168.0.31", port=7362)  # <-- Constructor call
    print(fldigi_client.name)
    print(fldigi_client.version)

    for m in fldigi_client.methods:
        print(m)
